...
import { Test, tests } from "../../data/tests";
import {
    chooseFiveMondai,
    getRandomMondai,
    chooseThreeDrag,
...
const TestScreen = () => {
    ...
    const savedTest = context.savedTest;
    ...
    const [test] = useState(() => {
        const index = Number(lecture?.lectureId);

        return tests[index] as Test;
    });
    ...
    const [hasLastTest] = useState(() => {
        ...
        const lastTest = user.currentProgress[index]?.["lastTest"];
    ...
    const [hasHighScore] = useState(() => {
        ...
        const highScore =
            user.currentProgress[index]?.["highScore"];
        if (highScore) {
    ...
    const [hasWonMedal] = useState(() => {
        ...
        const hasMedal = user.currentProgress.stickers?.[index];
    ...
    const updateTestTime = (info: SetStateAction<{ totalSeconds: number; seconds: number; minutes: number; hours: number; }>) => {
        setTimerInfo(info);
    };

    // const handleOptionClick = (info: { correct: any; index?: any; prompt?: any; expected?: any; answer?: any; }) => {
    //     if (info.correct) {
    ...
    // const handleDragAnswer = (info: { correct: any; prompt?: any; expected?: any; answer?: any; }) => {
    //     if (info.correct) {
    //         //console.log("Correct");
    ...
    // const writeAnswer = (type, info: { prompt: any; expected: any; answer: any; correct: any; }) => {
    //     setAnswers({
    //         ...answers,
    //         [type]: [
    //             ...answers[type],
    ...
    const handleBeginButtonClick = (stage: SetStateAction<string>) => {
        if (stage === "mondai") {
            setNewRecord(false);
            setStartTest(true);
    ...
    const handleSaveTestScore = () => {
        ...
        if (score > previousHighScore) {
            setNewRecord(true);
            payloadObject.currentProgress[index].highScore = {
                ...answers,
                score: { [test.version]: score },
                timer: timerInfo,
            };
